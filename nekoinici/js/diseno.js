	...
	function deshabilitar()
	{
        ...
        function obtenerTooltipSlider($input, target)
        {
                var $tooltip=$();
                if(target)
                {
                ...
                if(!$tooltip.length)
                {
                        $tooltip=$input.siblings(".slider-tooltip");
                }
                if(!$tooltip.length)
                {
                        $tooltip=$input.closest(".slider-control").find(".slider-tooltip").first();
                }
                return $tooltip;
        }

        function posicionarTooltipSlider($input, $tooltip, porcentaje)
        {
                if(!$tooltip.length)
                {
                        return;
                }
                var porcentajeAjustado=Math.max(5, Math.min(95, porcentaje));
                $tooltip.css("left", porcentajeAjustado+"%");
        }

        function alternarTooltipSlider($tooltip, visible)
        {
                if(!$tooltip.length)
                {
                        return;
        ...
        function actualizarSliderPorcentaje(input, target)
        {
                var $input=$(input);
                if(!$input.length)
                {
                ...
                var $tooltip=obtenerTooltipSlider($input, target);
                var valor=parseFloat($input.val());
                if(isNaN(valor))
                {
                        var minAttr=$input.attr("min");
                        valor=(minAttr!==undefined) ? parseFloat(minAttr) : 0;
                }
                ...
                var porcentaje=(max!==min) ? ((valor-min)/(max-min))*100 : 0;
                posicionarTooltipSlider($input, $tooltip, porcentaje);
        }

        function inicializarSlidersPorcentaje()
        {
                actualizarSliderPorcentaje("#id_width","#id_width_value");
                actualizarSliderPorcentaje("#id_width_imt","#id_width_imt_value");
                actualizarSliderPorcentaje("#id_width_img_gif","#id_width_img_gif_value");
        }

        function vincularSliderTooltip(config)
        {
                ...
                var eventosMostrar="focus pointerdown mouseenter touchstart";
                var eventosOcultar="blur mouseleave touchend touchcancel pointerleave";

                $contenedor.on("input change", config.selector, function(){
                        var $input=$(this);
                        var $tooltip=obtenerTooltipSlider($input, config.tooltip);
                        actualizarSliderPorcentaje($input, $tooltip);
                        if(typeof config.onUpdate === "function")
                        {
                                config.onUpdate.call(this, $input);
                        }
                });

                $contenedor.on(eventosMostrar, config.selector, function(){
                        var $input=$(this);
                        var $tooltip=obtenerTooltipSlider($input, config.tooltip);
                        actualizarSliderPorcentaje($input, $tooltip);
                        alternarTooltipSlider($tooltip, true);
                });

                $contenedor.on(eventosOcultar, config.selector, function(){
                        var $input=$(this);
                        var $tooltip=obtenerTooltipSlider($input, config.tooltip);
                        alternarTooltipSlider($tooltip, false);
                });
        }

        $(function(){
                inicializarSlidersPorcentaje();

                vincularSliderTooltip({
                        contenedor: "#config_modulos_imagen",
                        selector: "#id_width",
                        tooltip: "#id_width_value",
                        onUpdate: function(){
                                if(typeof poner_imagen === "function")
                                {
                                        poner_imagen();
                                }
                        }
                ...
                vincularSliderTooltip({
                        contenedor: "#config_modulos_imagen_texto",
                        selector: "#id_width_imt",
                        tooltip: "#id_width_imt_value",
                        onUpdate: function(){
                                if(typeof poner_imagen_imt === "function")
                                {
                                        poner_imagen_imt('0');
                                }
                        }
                ...
                vincularSliderTooltip({
                        contenedor: "#config_modulos_img_gif",
                        selector: "#id_width_img_gif",
                        tooltip: "#id_width_img_gif_value",
                        onUpdate: function(){
                                if(typeof poner_img_gif === "function")
                                {
                                        poner_img_gif();
                                }
                        }
...
*/
	...
	$("#guardar_tex").on("click", function(event){
		...
		pcampana=$('#id_campana').val();
                ...
                console.log('He clickado TEX (1) '+diseno+' (2) '+valor_id+' (3) '+tipo+' (4)*'+valor_line+'* (5)'+valor_ids+' (6)'+orden);
                        ...
                        type: 'post',
                         url: 'guardar_modulos.php',
                               ...
                               success: function(response){
                                        ...
                                        //wid=size*pid_width/100;
					let altEscapado = palt_imagen.replaceAll("'", "&apos;");
	...
	$("#guardar_bot").on("click", function(event){
	 ...
	 //Boton Guardar Módulos de Texto
		...
		tpos=$('input:radio[name=id_posicion_imt]:checked').val(); console.log('tpos '+tpos);
                ...
                console.log('He clickado IMG'+diseno+' '+valor_id+' '+tipo+' '+valor_line+' '+pimagen+' '+plink_imagen+' '+palt_imagen+' '+palinear_imagen+' '+size);
                console.log('He clickado IMG'+palt_imagen);
                ...
                $.ajax({
					...
					*/
					...
					console.log('palt imagen: '+palt_imagen);
					//let palt_imagen = palt_imagen.replaceAll("'", "&apos;");
					let palt_imagenes = palt_imagen.replaceAll("'", "&apos;");
					if(tpos==1)
			                { //left
                        			poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' style='max-width:600px' onclick='editar_general()'><tr><td width='50%'><p "+pestilo+"><img src='"+pimagen+"' alt='"+palt_imagenes+"' style='display:block; margin:0px auto;' data-link_imagen='"+plink_imagen+"' data-alinear_imagen='"+palinear_imagen+"' data-width='"+size+"' width='"+wid+"' data-ids='"+ids+"' data-tanto='"+pid_width+"' data-posicion='"+tipo+"' data-id='"+ids+"' id='"+ids+"'></p></td><td width='50%'><span>"+ptexto+"</span></td></tr></table>";
			                }
			                else
			                {
			                        if(tpos==2)
			                        {//right
                        			        poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' style='max-width:600px' onclick='editar_general()'><tr><td width='50%'><span>"+ptexto+"</span></td><td width='50%'><p "+pestilo+"><img src='"+pimagen+"' alt='"+palt_imagenes+"' style='display:block; margin:0px auto;' data-link_imagen='"+plink_imagen+"' data-alinear_imagen='"+palinear_imagen+"' data-width='"+size+"' width='"+wid+"' data-ids='"+ids+"' data-tanto='"+pid_width+"' data-posicion='"+tipo+"' data-id='"+ids+"' id='"+ids+"'></p></td></tr></table>";
			                        }
                        			else
			                        {
                        			        if(tpos==3)
                        			        ...
                        			        else
			                                {
                        			                if(tpos==4)
			                                        {//right
                        			                        poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' style='max-width:600px' onclick='editar_general()'><tr><td width='50%'><span>"+ptexto+"</span></td></tr><tr><td width='50%'><p "+pestilo+"><img src='"+pimagen+"' alt='"+palt_imagenes+"' style='display:block; margin:0px auto;' data-link_imagen='"+plink_imagen+"' data-alinear_imagen='"+palinear_imagen+"' data-width='"+size+"' width='"+wid+"' data-ids='"+ids+"' data-tanto='"+pid_width+"' data-posicion='"+tipo+"' data-id='"+ids+"' id='"+ids+"'></p></td></tr></table>";
			                                        }
                        			        }
			                        }
			                }
	...
	$("#enviar_ahora").on("click", function(event){
                ...
                pcampana=$("#id_campana").val();
                psmtp2go=$("#id_smtp2go").val();
                ppartner=$("#id_partner").val();
		orden=$('#dropfinal3').attr("data-quien");
...
*/
				...
				pdata.append('id_smtp2go',psmtp2go);
				pdata.append('id_partner',ppartner);
					 ...
					 var obj = JSON.stringify(response);
						 ...
						 console.log(objs.message);
                				ptexto9=$("#ptexto9").val(); 
                                       		...
                                       		$('.myModalN_body').html('<div style="text-align:center"><img src="../img/OK.png"><br><br>'+ptexto9+'.</div>');
					        $("#myModalN").modal("show");
					 }
                                 },
	...
	$("#seguir").on("click", function(event){
		...
		$.ajax({
				     ...
				     $("#id_nombre_camp").val(pdencam);
                                     ...
                                     console.log('Resp: *'+response+'*\n');
                                     $('.nav-tabs #tab_config').css('display', 'block');
				     if(ok_hoy==0 && ok_men==0) $("#enviar_ahora").removeAttr('disabled');
                                     if(ok_men==0) $("#programar").removeAttr('disabled');
                               },
                        error: function() {
                                  console.log('Error en ajax intercambio');
                              }
			...
			ptexto8=$("#ptexto8").val(); 
                ...
                else
                {
                        if(valid==1 && validh)
                        {
                                $.ajax({
                                type: 'post',
                                url: '../envio/enviar_email_testeo.php',
                                //url: '../envio/enviar_email_test.php',
                                data:{id_profile:pprofile,datos:pdatos,id_diseno:pdiseno,id_asunto:pasunto,id_nombre:pnombre,id_preheader:ppreheader,id_campana:pcampana,id_remitente:premitente,id_html:phtml,id_testeo:ptesteo},
					...
					ptexto13=$("#ptexto13").val(); 
					ptexto14=$("#ptexto14").val(); 
					ptexto15=$("#ptexto15").val(); 
                                       //$('.myModalE_body').html('<span style="color:green">Su correo de test se ha enviado correctamente.<br><br> Este correo de test se está enviando desde una dirección de correo electrónico de Neko Mailing. Su campaña se enviará correctamente con su dirección corporativa cómo remitente.</span>');
                                       $('.myModalE_body').html('<span style="color:green">'+ptexto13+'.<br><br> '+ptexto14+'. '+ptexto15+'.</span>');
                                       $("#myModalE").modal("show");
                                },
                                error: function() {
                                       console.log('Error en ajax intercambio');
	...
	$("#seguirs").on("click", function(event){
		...
		//cerrar modal
	        ...
	        //$('#myModalCams').modal('toggle');
                ...
                pcampana=$('#id_campana').val();
                $('#id_nombre_camp').val(pdencamp);
		ptexto11=$("#ptexto11").val(); 
	        //pnomcam="<span>Nombre de la Campaña: "+pdencamp+"</span>";
	        pnomcam="<span>"+ptexto11+": "+pdencamp+"</span>";
                $('#pnomcam').html(pnomcam);
			 ...
			 dataType: "json",
				       ...
				       if(response.ok)
				       {
					       ...
					       if(tipo=="texto")
					       {
							...
							ptexto8=$("#ptexto8").val(); 
							if(this.Texto=="")
								pvalor = "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width='100%' onclick='editar_general()'><tr><td><span>"+ptexto8+"</span></td></tr></table>"; //Texto
					       ...
					       if(tipo=="imagen")
							...
							ptexto4=$("#ptexto4").val(); 
                                                        if(this.Imagen=="")
                                                                pvalor = "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width='100%' onclick='editar_general()'><tr><td><p style='text-align:center;margin-bottom:0px;'><img src='../img/imagen.png' alt='"+ptexto4+"' style='display:block; margin:0px auto;' data-link_imagen='' data-alinear_imagen='' data-width='100' data-ids='"+this.Codigo+"' data-tanto='100' data-id='"+this.Codigo+"' id='"+this.Codigo+"'>Subir una imagen</p></td></tr></table>"; //imagen
                                                        else
						        {
								let altEscapado = this.Alt.replaceAll("'", "&apos;");
                                                                pvalor = "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width='100%' onclick='editar_general()'><tr><td><p style='text-align:"+this.Alinear+";margin-bottom:0px;'><img src='"+this.Imagen+"' alt='"+altEscapado+"' data-link_imagen='"+this.Link+"' data-alinear_imagen='"+this.Alinear+"' data-width='"+this.Width+"' data-ids='"+this.Codigo+"' width='"+wid+"' data-tanto='"+this.Tanto+"' data-id='"+this.Codigo+"' id='"+this.Codigo+"'></p></td></tr></table>"; //imagen
						        }
                                                        nodeCopy.innerHTML = suma+pvalor;
					       ...
					       if(tipo=="imagen_texto")
							...
							var nodeCopy = document.createElement('div');
							wid=this.Width*this.Tanto/100; 
						        if(wid>300) 
							       wid=300*this.Tanto/100;
							...
							if(tpos==0) tpos=3;
                		                        if(tpos==1)
                                		        { //left
                                                		poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' onclick='editar_general()'><tr><td width='50%'><p "+pestilo+"><img src='"+this.Imagen+"' alt='"+altEscapado+"' style='display:block; margin:0px auto;' data-link_imagen='"+this.Link+"' data-alinear_imagen='"+this.Alinear+"' data-width='"+this.Width+"' width='"+wid+"' data-ids='"+this.Codigo+"' data-tanto='"+this.Tanto+"' data-posicion='"+this.Posicion+"' data-id='"+this.Codigo+"' id='"+this.Codigo+"' ></p></td><td width='50%'><span>"+this.Texto+"</span></td></tr></table>";
		                                        }
                		                        else
                                		        {
                                                		if(tpos==2)
		                                                {//right
                		                                        poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' onclick='editar_general()'><tr><td width='50%'><span>"+this.Texto+"</span></td><td width='50%'><p "+pestilo+"><img src='"+this.Imagen+"' alt='"+altEscapado+"' style='display:block; margin:0px auto;' data-link_imagen='"+this.Link+"' data-alinear_imagen='"+this.Alinear+"' data-width='"+this.Width+"' width='"+wid+"' data-ids='"+this.Codigo+"' data-tanto='"+this.Tanto+"' data-posicion='"+this.Posicion+"' data-id='"+this.Codigo+"' id='"+this.Codigo+"'></p></td></tr></table>";
                                		                }
                                                		else
		                                                {
                		                                        if(tpos==3)
		                                                        ...
		                                                        else
                		                                        {
                                		                                if(tpos==4)
                                                		                {//right
                                                                		        poner="<table border='0' cellspacing='0' cellpadding='0' width='100%' onclick='editar_general()'><tr><td width='50%'><span>"+this.Texto+"</span></td></tr><tr><td width='50%'><p "+pestilo+"><img src='"+this.Imagen+"' alt='"+altEscapado+"' style='display:block; margin:0px auto;' data-link_imagen='"+this.Link+"' data-alinear_imagen='"+this.Alinear+"' data-width='"+this.Width+"' width='"+wid+"' data-ids='"+this.Codigo+"' data-tanto='"+this.Tanto+"' data-posicion='"+this.Posicion+"' data-id='"+this.Codigo+"' id='"+this.Codigo+"'></p></td></tr></table>";
		                                                                }
                		                                        }
								}
                                        		}
					       ...
					       if(tipo=="redes_sociales")
					       {
							...
							//id_contar++; //Modificado el 16-03-2025
							if(this.URed!="") { id_contar++; id_facebook="<td><div style='padding:5px;text-align:center'><img src='../img/icono_facebook.png' width='32' height='32' data-facebook='"+this.URed+"'></div></td>"; } else id_facebook="";
						        //console.log(this.URed+" "+id_facebook);
