	...
	var pdatos=$("#datos").val(); 
        ...
        $.ajax({
			...
			$(".dropzone").css("cursor", "not-allowed");
				...
				//$('.alert').html('<strong>Fichero subido correctamente</strong>'+objs.idfich);
...
$('#copia_pega').click(function () {
	$('.subir_fichero_pegar').css("display", "block");
})


$('.subir_fichero').click(function () {
        if( $('#lssi').prop('checked') )
	{
		$("#form_fichero").submit();
	}
	else
	{
	 	$(".myModal_body").html('Debe aceptar la ley LSSI');
                $("#myModal").modal("show");
	}
})

$('.subir_fichero_pegar').click(function () {
        if( $('#lssi').prop('checked') )
	{
		$("#form_fichero_pegar").submit();
	}
	else
	{
	 	$(".myModal_body").html('Debe aceptar la ley LSSI');
